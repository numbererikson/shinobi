// @shinobi fitness reference plugin (mission type: fitness_180).
//
// This is the reference implementation for Shinobi's writable plugin state
// (`registry.state`). It demonstrates long-term adaptive mission state: the
// plugin owns structured facts, restrictions, IDs and snapshots; the LLM owns
// all language. The plugin never returns coaching or motivational text.
//
// It lives in-tree for strict typechecking and test coverage and is NOT
// auto-loaded into a running server (discovery only scans node_modules and
// ~/.shinobi/plugins). Publishing as @shinobi/plugin-fitness is a thin
// extraction that re-exports `register`.
